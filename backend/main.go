...
func main() {
	...
	mux.HandleFunc("/", cors(spaHandler))
...
func spaHandler(w http.ResponseWriter, r *http.Request) {
	if strings.HasPrefix(r.URL.Path, "/api") || strings.HasPrefix(r.URL.Path, "/api-proxy") || strings.HasPrefix(r.URL.Path, "/ws-proxy") {
		http.NotFound(w, r)
		return
	}

	filePath := filepath.Join(staticDir, path.Clean(r.URL.Path))
...
func analyzeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if config.GeminiApiKey != "" {
		if data, err := callGeminiAnalyze(r.Context(), req); err == nil {
...
func callGeminiAnalyze(ctx context.Context, req analyzeRequest) ([]byte, error) {
	body := map[string]any{
	...
	if config.GeminiApiKey == "" {
		return nil, fmt.Errorf("GEMINI_API_KEY must be configured")
	}

	endpoint := fmt.Sprintf("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=%s", config.GeminiApiKey)
	forwardReq, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(bodyBytes))
	if err != nil {
		return nil, err
	}
